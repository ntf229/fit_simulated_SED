...
res, obs, model = reader.results_from(args.filename)
...
file = open("bestfit_params.txt", "w") 
...
rfw = np.load('full_rf_wavelengths.npy') # generated from sdss_bands_test read.py

mask = ((a * rfw) >= 1e3) & ((a * rfw) <= 1e8)

plt.figure(figsize=(10,8))

wave_correct = np.load('wave.npy') # in microns
wave_correct = wave_correct * 1e4 # convert to Angstroms
spec_correct = np.load('spec.npy') # units of Jy 
